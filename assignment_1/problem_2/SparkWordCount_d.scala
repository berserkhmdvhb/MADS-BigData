...
object SparkWordCount_d {
  def main(args: Array[String]): Unit = {
    if (args.length < 3) {
      System.err.println("Correct arguments: <input-directory> <output-directory> <stopwords.txt>")
      System.exit(1)
    }
    ...
    val stopwords = ctx.broadcast(Source.fromFile(args(2)).getLines.toList) // this should now be a list of the stopwords
    // stopwords.foreach {println}

    // first define the patterns we want
    // make them filter functions 
    // TODO: adjust the patterns Hamed/Maarts pls
    def is_stopWord(token: String): Boolean = {
      stopwords.value.find(t => t.equals(token)) match {
    ...
    def is_word(token: String): Boolean = {
      val pattern = "(?>[a-z-_]*)".r
      return pattern.pattern.matcher(token).matches && !is_stopWord(token)
    }
